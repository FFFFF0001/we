		...
		<div id="main">
			<div id="left" class="col-xs-12">
				<div id="top" class="clearfix">
					<div id="top_left" class="col-xs-12 col-sm-4"><img src="<?php echo sp_get_asset_upload_path('activity_cover/').$details['cover'];?>" class="img-responsive"></div>
					<div id="top_right" class="col-xs-12 col-sm-8">
						<h1><b><?php echo ($details["activity_name"]); ?></b></h1>

						<p>时间：<?php echo date("Y-m-d H:i:s",$details['start_time']);?> -- <?php echo date("Y-m-d H:i:s",$details['end_time']);?></p>
						<p class="jieduan">地点：<?php echo ($details["area"]); ?></p>
						<p>分类：<a href="#"><?php echo ($details["type"]); ?></a></p>
						<?php if($details.charge==0){ $charge = "免费"; } else{ $charge = $details.charge."元"; } ?>
						<p>费用：<?php echo ($charge); ?></p>
						<p class="top_right_bottom jieduan">发起组织：<a href="#"><i></i> <?php echo ($details["manager_name"]); ?></a></p>

					</div>
				</div>
				<p class="top_next">
					<a href="#" class="top_next_first">活动详情</a>
					<a href="#">活动回顾</a>
				</p>
				<div class="center col-xs-11">
					<p class="activityDetail-title">活动摘要：</p>
					<?php echo ($details["introduction"]); ?>
				...
				<p style="padding-bottom:10px;"><span style="color:green;">报名情况</span>：<?php echo ($baomingsCount); ?>人报名</p>
				<ul class="d-avatar-ul">
					<?php if(is_array($baomings)): foreach($baomings as $key=>$vo): ?><li class="li_p">
							<a href=""><img src="<?php echo sp_get_asset_upload_path('avatar/').$vo['avatar'];?>" alt="">
								<p class="jieduan"><?php echo ($vo["user_nicename"]); ?></p>
							</a>
			...
			<div id="right">
				...
				<div id="right_second">
					...
					<div class="three">
						<p>&nbsp36</p>
						<!--判断是否参加过-->
						<?php if($isbaoming == true): ?><div class="three_one_joined"></div>
							<?php else: ?>
							<a href="<?php echo U('join',array('id'=>$details['id']));?>">
				...
				<div id="wrapper-act-active">
					<h3 class="title-act-active">活动动态</h3>
					<?php if(is_array($baomings)): foreach($baomings as $key=>$voo): ?><div class="right_third">
							<div class="right_third_img"><img src="<?php echo sp_get_asset_upload_path('avatar/').$voo['avatar'];?>" alt=""></div>
							<div class="right_third_p">
								<p class="r-avatar-username jieduan"><?php echo ($voo["user_nicename"]); ?></p>
								<p class="right_third_span_font">参加了这个活动</p>
							</div>
							<span class="right_third_span">3小时前</span>
							<!--<?php echo date("Y-m-d H:i:s",$voo['join_time']);?>-->
