...
<body>
	<div class="wrap">
		<div id="error_tips">
			<h2>
				<?php if(empty($errormenus)): ?>菜单导入成功！
				<?php else: ?>
					菜单导入失败！<?php endif; ?>
			</h2>
			<div class="error_cont">
				<ul>
					<?php if(empty($errormenus)): ?>菜单导入成功！
					<?php else: ?> 
						<?php if(is_array($errormenus)): foreach($errormenus as $key=>$vo): ?><li><?php echo ($vo); ?>导入失败!</li><?php endforeach; endif; endif; ?>
				</ul>
				<div class="error_return">
					<a href="<?php echo U('menu/index');?>" class="btn">返回</a>
				</div>
			</div>
		</div>
	</div>
	<script src="/2016Sise/public/js/common.js"></script>
	<?php if(empty($errormenus)): ?><script>
